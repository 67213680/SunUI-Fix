...
local function BuildClock()
	...
	local week,killsoa, nokillsoa,killpj,nokillpj,killodst,nokillodst,killnlk,nokillnlk
	if (GetLocale() == "zhTW" or GetLocale() == "zhCN") then
		week = {"星期天","星期一","星期二","星期三","星期四","星期五","星期六"}
		killsoa = "本周|cff228B22已经击杀|r怒之煞"
		nokillsoa = "本周还|cffFF0000没有击杀|r怒之煞"
		killpj = "本周|cff228B22已经击杀|r炮舰"
		nokillpj = "本周还|cffFF0000没有击杀|r炮舰"
		killodst = "本周|cff228B22已经击杀|r乌达斯塔"
		nokillodst = "本周还|cffFF0000没有击杀|r乌达斯塔"
		killnlk = "本周|cff228B22已经击杀|r纳拉克"
		nokillnlk = "本周还|cffFF0000没有击杀|r纳拉克"
	else
		week = {"Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"}
		killsoa = "You|cff228B22killed|r SOA in this week"
		nokillsoa = "You |cffFF0000no kill|r SOA in this week"
	end
	...
	Clock:SetScript("OnEnter", function(self)
		...
		if UnitLevel("player") > 89 then
			...
			if isCD then
				GameTooltip:AddLine(killpj, 0.75, 0.9, 1)
			else
				GameTooltip:AddLine(nokillpj, 0.75, 0.9, 1)
			end
		end
		if UnitLevel("player") > 89 then
			local isCD = IsQuestFlaggedCompleted(32519)
			if isCD then
				GameTooltip:AddLine(killodst, 0.75, 0.9, 1)
			else
				GameTooltip:AddLine(nokillodst, 0.75, 0.9, 1)
			end
		end
		if UnitLevel("player") > 89 then
			local isCD = IsQuestFlaggedCompleted(32518)
			if isCD then
				GameTooltip:AddLine(killnlk, 0.75, 0.9, 1)
			else
				GameTooltip:AddLine(nokillnlk, 0.75, 0.9, 1)
			end
		end
		GameTooltip:Show()
	end)
